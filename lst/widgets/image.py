...
class Image(Gtk.Image, Widget):
    ...
    def image(self, value: Union[str, GdkPixbuf.Pixbuf]) -> None:
        if isinstance(value, GdkPixbuf.Pixbuf):
            ...
            self.set_from_icon_name(icon_name='image-missing', size=Gtk.IconSize.from_name('image-missing'))
            

        self._image = value
        self.size = self.size

    def set_image(self, value: Union[str, GdkPixbuf.Pixbuf]) -> None:
        self.image = value
    ...
    def size(self, value: int) -> None:
        if value:
            if isinstance(self._image, GdkPixbuf.Pixbuf):
                scalled_image = self._image.scale_simple(value, value, GdkPixbuf.InterpType.BILINEAR)
                self.set_from_pixbuf(pixbuf=scalled_image)

            elif isinstance(self._image, str):
                if os.path.isfile(self._image):
                    scalled_image = GdkPixbuf.Pixbuf.new_from_file(self._image).scale_simple(value, value, GdkPixbuf.InterpType.BILINEAR)
                    self.set_from_pixbuf(pixbuf=scalled_image)
                else:
